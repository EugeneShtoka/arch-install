...
echo "Info: Successfully created branch and merge/pull request."#!/bin/zsh
...
git switch "$GIT_DEFAULT_BRANCH" || {
	echo "Error: git switch $GIT_DEFAULT_BRANCH failed." >&2
	exit 1
}
...
echo "Info: Pulling latest changes for '$GIT_DEFAULT_BRANCH'..."
git pull || {
	echo "Error: git pull on $GIT_DEFAULT_BRANCH failed." >&2
	exit 1
}

echo "Info: Creating and checking out new branch '$branchName'..."
git checkout -b "$branchName" || {
	echo "Error: git checkout -b $branchName failed." >&2
	exit 1
}

echo "Info: Applying stashed changes..."
git stash apply || {
	echo "Error: 'git stash apply' failed. Please resolve conflicts or issues manually and try again." >&2
	exit 1
}
