...
if [[ -n $title ]]; then
    ...
    resJson=$(gemini-cli-wrapper "From title get artist and song: '$title', json response, response in single line")
    echo "test"
    if [[ -n $resJson ]]; then
        ...
        if [[ -n $artist && -n $song ]]; then
            ...
            notify_send "YouTube Download" "Track saved to Music/$artist/$song.mp3"
        fi
    fi
fi
