...
function connectToMeeting() {
	...
	if [[ "$conferenceType" == "Zoom Meeting" ]]; then
		...
		setsid setsid $BROWSER "$url" > /dev/null 2>&1 &
	fi
}
...
if [[ $meetingCount -eq 0 ]]; then
	...
	notify_send "Auto Join meetings" "<span font='20px'>No meetings scheduled soon.<br>Next meeting:<br>  $(formatMeeting $meeting $formatMeeting)</span>" --icon " " -r 101023
	...
	choice=$(printf '%s\n' "${availableMeetings[@]}" | $SCRIPTS_PATH/rofi-run.sh -theme ${dir}/${theme}.rasi -dmenu -matching prefix)
	echo "$meetings" | jq -c '.[]' | while read meeting; do
		name=$(formatMeeting $meeting $formatMeeting)
		if [[ "$name" == "$choice" ]]; then
			$(connectToMeeting "$meeting")
