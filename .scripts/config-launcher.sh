...
sudo $SCRIPTS_PATH/replace-line.sh "Exec=" "Exec=mailspring --password-store='gnome-libsecret' %U" "$APPS_PATH/Mailspring.desktop"
...
sudo $SCRIPTS_PATH/replace-line.sh "Exec=" "Exec=$SCRIPTS_PATH/start-dbeaver.sh" "$APPS_PATH/io.dbeaver.DBeaver.desktop"
