...
if [[ "$IS_LAPTOP" = "1" ]]; then
  get_battery_info
  hardware_info="$(get_battery_status $battery_level $battery_state)$cpu_and_ram $hardware_info"
fi
...
notify_send "$(date +%H:%M)" "$message" --icon=" " -r 101039 -a "system-notify"
